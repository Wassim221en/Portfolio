...
function getRankColor(rating: number | null): { color: string; rank: string; bgColor: string } {
  if (rating === null) return { color: 'text-gray-400', rank: 'Unrated', bgColor: 'bg-gray-100' };
  if (rating < 1200) return { color: 'text-gray-600', rank: 'Newbie', bgColor: 'bg-gray-100' };
  if (rating < 1400) return { color: 'text-green-600', rank: 'Pupil', bgColor: 'bg-green-50' };
  if (rating < 1600) return { color: 'text-cyan-600', rank: 'Specialist', bgColor: 'bg-cyan-50' };
  if (rating < 1900) return { color: 'text-blue-600', rank: 'Expert', bgColor: 'bg-blue-50' };
  if (rating < 2100) return { color: 'text-purple-600', rank: 'Candidate Master', bgColor: 'bg-purple-50' };
  if (rating < 2300) return { color: 'text-orange-500', rank: 'Master', bgColor: 'bg-orange-50' };
  if (rating < 2400) return { color: 'text-orange-600', rank: 'International Master', bgColor: 'bg-orange-100' };
  if (rating < 2600) return { color: 'text-red-500', rank: 'Grandmaster', bgColor: 'bg-red-50' };
  return { color: 'text-red-700', rank: 'Legendary Grandmaster', bgColor: 'bg-red-100' };
}
...
function CodeforcesStatsCard() {
  ...
  if (loading) {
    return (
      <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-100 p-8 animate-pulse">
  ...
  if (error) {
    return (
      <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-red-100 p-8 text-center">
        <div className="text-red-500 text-6xl mb-4">⚠️</div>
        <h3 className="text-xl font-semibold text-gray-800 mb-2">Unable to load stats</h3>
  ...
  return (
    <ScrollAnimation>
      ...
      <h1 className="text-3xl font-bold text-gray-800 mb-4 px-4">Codeforces Statistics</h1>
    </div>
      <div className="w-full max-w-6xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        {/* Header */}
        <div className={`${bgColor} px-8 py-6 border-b border-gray-100`}>
        ...
        <div className="p-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className={`text-center p-4 ${bgColor} rounded-lg border ${color.replace('text-', 'border-')}`}>
            ...
            <div className="text-center p-4 bg-orange-50 rounded-lg border border-orange-100">
              ...
              <div className="text-sm text-orange-700 font-medium flex items-center justify-center gap-1">
